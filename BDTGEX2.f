      SUBROUTINE BDTGEX2( N, A, LDA, B, LDB, J1, N1, N2, ITRAF, QTRAF,
     $                    ZTRAF, WORK, LWORK, INFO )
	IMPLICIT NONE
*
...
*
*     .. Scalar Arguments ..
      INTEGER            INFO, J1, LDA, LDB, LWORK, N, N1, N2
*     ..
*     .. Array Arguments ..
      INTEGER            ITRAF( * )
...
*
*  In contrast to the LAPACK routine DTGEX2, the orthogonal
*  transformation matrices Q and Z are not explicitely constructed but
*  represented by paramaters contained in the arrays ITRAF, QTRAF,
*  and ZTRAF, see the description of BDTGEXC for more details.
*
...
*
*  ITRAF   (output) INTEGER
*          Parameter for representing the transformation matrices Q and
*          Z, see BDTGEXC.
*
*  QTRAF   (output) DOUBLE PRECISION array, length k, where
*             k =  2,               if N1+N2 = 2;
*             k =  (N1+N2)*(N1+N2), if N1+N2 > 2.
*          List of parameters for representing the transformation
*          matrix Q, see BDTGEXC.
*
*  ZTRAF   (output) DOUBLE PRECISION array, length k, where
*             k =  2,               if N1+N2 = 2;
*             k =  (N1+N2)*(N1+N2), if N1+N2 > 2.
*          List of parameters for representing the transformation
*          matrix Z, see BDTGEXC.
*
...
*
         ITRAF( 1 ) = (M-2)*N + J1
	   CALL DLACPY( 'All', M, M, LI, LDST, QTRAF, M )
	   CALL DLACPY( 'All', M, M, IR, LDST, ZTRAF, M )
*
*        Update (A(J1:J1+M-1, M+J1:N), B(J1:J1+M-1, M+J1:N)) and
*                (A(1:J1-1, J1:J1+M), B(1:J1-1, J1:J1+M)).
*
...
*
      INFO = 1
c      write(*,*), '%%% REORDERING FAILS !!!'
c      call doutput(n1+n2, n1+n2, a(j1, j1), lda)
c      call doutput(n1+n2, n1+n2, b(j1, j1), ldb)
      RETURN
*
*     End of DTGEX2
*
