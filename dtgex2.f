...
*     .. Local Scalars ..
      LOGICAL            STRONG, WEAK
      INTEGER            I, J, IDUM, LINFO, M
      DOUBLE PRECISION   BIGNUM, BQRA21, BRQA21, DDUM, DNORM, DSCALE,
     $                   DSUM, EPS, F, G, MA, MB, SA, SB, SCALE, SMLNUM,
     $                   SS, THRESH, WS
*     ..
*     .. Local Arrays ..
      INTEGER            IWORK( LDST )
...
*     .. External Subroutines ..
      EXTERNAL           DGEMM, DGEQR2, DGERQ2, DLABAD, DLACPY, DLAGV2,
     $                   DLARTG, DLASET, DLASSQ, DORG2R, DORGR2, DORM2R,
     $                   DORMR2, DROT, DSCAL, DTGSY2
...
c      THRESH = MAX( TEN*EPS*DNORM, SMLNUM )
*
      IF( M.EQ.2 ) THEN
*
...
*
     ...
     $                   M )
            IF( MA.GT.ZERO ) THEN
               DDUM = ONE/MA
               CALL DSCAL( M*M, DDUM, WORK( M*M+1 ), 1 )
            END IF
...
*
     ...
     $                LINFO )
c      call doutput(n1, n2, ir(n2+1, n1+1), ldst)
c      call doutput(n1, n2, li, ldst)
*
...
*
c         CALL DLACPY( 'F', M, M, S, LDST, A( J1, J1 ), LDA )
c         CALL DLACPY( 'F', M, M, T, LDST, B( J1, J1 ), LDB )
         DO J = 1, M
            DO I = 1, M
               A( J1-1+I, J1-1+J ) = S(I, J) * MA
               B( J1-1+I, J1-1+J ) = T(I, J) * MB
            END DO
         END DO
